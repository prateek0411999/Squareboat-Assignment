...
import Recruiter from '../modals/recruiter.js';
...
export const signup = async (req, res) => {
    ...
    try {
        ...
        if (existingUser) return res.status(400).json({ message: "Recruiter already exists" });

        if (password !== confirmPassword) return res.status(400).json({ message: 'Password doest not match' })

        const hashedPassword = await bcrypt.hash(password, 12);

        const result = await Recruiter.create({ email, password: hashedPassword, name: `${firstName} ${lastName}`, userType });
